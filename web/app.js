...
// ============================================
// FIX: Route Drawing — use stable OSRM endpoint
// The default router hits router.project-osrm.org which is rate-limited and
// often returns no response in browser contexts, causing silent failures where
// the routing control is added but draws nothing — only markers are visible.
// Both functions now pass `router: osrmRouter` (defined at the top of the file)
// pointing to routing.openstreetmap.de, which is stable and CORS-friendly.
// ============================================
...
function drawFinalRoute() {
    ...
    let assignedAmb = ambulances.find(a => a.id == StateHub.assignedAmbId) || ambulances[0];
    let hospital    = graphNodes.find(n => n.id == StateHub.selectedHospitalId);
